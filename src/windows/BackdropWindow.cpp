...
void BackdropWindow::waitForResize(LONG left, LONG top) const
{
    ...
    for (int x = 0; x < 66; x++)
    { // capping out at 330 ms, which is already fairly slow
...
BackdropWindow::BackdropWindow(COLORREF color, const char *className) : Window(CreateSolidBrush(color), className, "Backdrop Window", WS_EX_TOOLWINDOW, WS_POPUP)
{
    m_color = color;
}
